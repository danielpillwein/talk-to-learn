...
import { Mic, Square, Loader2, CheckCircle2, RotateCcw, XCircle, RefreshCw, FileText, ArrowLeft, User } from 'lucide-react';
...
export default function Home() {
    ...
    if (!selectedFile) {
        return (
            <div className="min-h-screen bg-[#f7f7fb] p-4 sm:p-6">
                <div className="max-w-5xl mx-auto space-y-6">
                    <div className="flex flex-col gap-4 pt-4 sm:flex-row sm:items-center sm:justify-between">
                        <div>
                            <p className="text-xs uppercase tracking-[0.2em] text-slate-400">Lernsets</p>
                            <h1 className="text-3xl font-bold text-slate-900">Talk to Learn</h1>
                            <p className="text-slate-500">Wähle ein Lernset aus</p>
                        </div>
                        <Link
                            href="/app/account"
                            className="flex w-fit items-center gap-2 rounded-full border border-slate-200 bg-white px-3 py-2 text-xs font-semibold text-slate-700 shadow-sm transition hover:border-slate-300 hover:text-slate-900"
                        >
                            {user?.image ? (
                                <img
                                    src={user.image}
    ...
    // --- VIEW 4: LEARNING ---
    return (
        <main className="min-h-screen bg-[#f7f7fb] p-4 sm:p-6">
            <div className="max-w-4xl mx-auto space-y-6">
                {/* Header */}
                <div className="flex flex-col gap-4 rounded-3xl border border-white/70 bg-white/80 p-4 shadow-sm sm:p-5">
                    <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                        ...
                        <div className="flex items-center gap-2">
                            <Link
                                href="/app/account"
                                className="flex items-center gap-2 rounded-full border border-slate-200 bg-white px-3 py-1.5 text-xs font-semibold text-slate-700 shadow-sm transition hover:border-slate-300 hover:text-slate-900"
                            >
                                {user?.image ? (
                                    <img
                                        src={user.image}
